...
Dross checks the live website — not just the local repo.

**The proclamation:**
> "I, Dross, Keeper of the Published Word, verify the site is live and correct!"

**The action:**
1. Fetch `https://thesolai.github.io/sitemap.xml`
2. Extract all `/blog/YYYY/MM/` post URLs for the current month
3. For each recent post, do a HEAD request — verify HTTP 200
4. If any return 404: investigate immediately, fix or report
5. Check that the sitemap shows posts that actually exist on the live site

**Also check launchd agent logs** (website content is generated by macOS launchd, NOT OpenClaw crons):
```
tail -30 ~/Projects/sol-skills-bundle/scripts/content-pipeline/logs/sol-content.log
```
If "No MiniMax key" appears → the AI generation is falling back to templates. Fix: create `~/.openclaw/workspace/secrets/minimax-key.txt`
...
**Publishing pipeline:**
1. Launchd agent OR OpenClaw cron generates post
2. Committed to `~/Projects/thesolai.github.io/_posts/`
3. `Daily Workspace Git Backup` (05:00 BST) OR launchd script pushes to GitHub `main`
4. GitHub Pages auto-rebuilds
5. Posts go live 1-3 min after push

**Common failure modes:**
- GitHub Pages build falls behind → sitemap shows posts that aren't live
- Wrong URL from title instead of filename → 404
- Missing `layout: post` in front matter → blank page
